...
$PyMain = Join-Path $RewardsDir "reward_ablation_actual_execution_release_manifest_design_step139.py"
$PyValidate = Join-Path $RewardsDir "validate_reward_ablation_actual_execution_release_manifest_design_step139.py"
$PyTest = Join-Path $RewardsDir "test_reward_ablation_actual_execution_release_manifest_design_step139.py"
$MdDoc = Join-Path $RewardsDir "reward_ablation_actual_execution_release_manifest_design_step139.md"
...
Step 139 designs the schema and template for a future actual execution release manifest.
...
Designed release matrix:
- condition: A only
- reward_ids: R0, R1, R2, R3, R4, R5
- seeds: 1, 2, 3
- planned_run_count: 18

A future release manifest must include explicit operator approval. Winner selection and reward promotion remain separate post-result-ingestion steps.
'@
...
$OutputRoot = "artifacts/rewards/reward_ablation_actual_execution_release_manifest_design_step139"
$Manifest = Join-Path $ProjectRoot "$OutputRoot\reward_ablation_actual_execution_release_manifest_design_step139_manifest.json"
$Template = Join-Path $ProjectRoot "$OutputRoot\actual_reward_ablation_release_manifest_template_step139.json"
...
if ($LASTEXITCODE -ne 0) {
    throw "[FAIL] Step 139 release manifest design failed"
}

& $py $PyValidate --manifest $Manifest --template $Template
if ($LASTEXITCODE -ne 0) {
    throw "[FAIL] Step 139 manifest/template validation failed"
}

& $py $PyTest
if ($LASTEXITCODE -ne 0) {
    throw "[FAIL] Step 139 self-test failed"
}

Write-Host "[DONE] Step 139 reward ablation actual execution release manifest design complete."
