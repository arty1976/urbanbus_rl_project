...
$PyMain = Join-Path $RewardsDir "concrete_release_manifest_draft_validator_gate_step142.py"
$PyValidate = Join-Path $RewardsDir "validate_concrete_release_manifest_draft_validator_gate_step142.py"
$PyTest = Join-Path $RewardsDir "test_concrete_release_manifest_draft_validator_gate_step142.py"
$MdDoc = Join-Path $RewardsDir "concrete_release_manifest_draft_validator_gate_step142.md"
...
Step 142 validates the Step 140 A-side concrete release manifest draft and the Step 141 B-group baseline reference manifest.

It confirms:
- A-side release draft exists and remains locked.
- B0R/B1/B2 baseline references are ready.
- actual_execution_allowed remains false.
- actual_execution_released remains false.
- actual_results remains false.
- winner_selected remains false.
- paper_level_claim_allowed remains false.
- causal_performance_claim_allowed remains false.
...
$OutputRoot = "artifacts/rewards/concrete_release_manifest_draft_validator_gate_step142"
$Manifest = Join-Path $ProjectRoot "$OutputRoot\concrete_release_manifest_draft_validator_gate_step142_manifest.json"
...
if ($LASTEXITCODE -ne 0) {
    throw "[FAIL] Step 142 concrete release manifest draft validator gate failed"
}

& $py $PyValidate --manifest $Manifest
if ($LASTEXITCODE -ne 0) {
    throw "[FAIL] Step 142 validator gate manifest validation failed"
}

& $py $PyTest
if ($LASTEXITCODE -ne 0) {
    throw "[FAIL] Step 142 self-test failed"
}

Write-Host "[DONE] Step 142 concrete release manifest draft validator gate complete."
