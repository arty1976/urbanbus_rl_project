...
$PyMain = Join-Path $RewardsDir "reward_ablation_command_runner_integration_step134.py"
$PyValidate = Join-Path $RewardsDir "validate_reward_ablation_command_runner_integration_step134.py"
$PyTest = Join-Path $RewardsDir "test_reward_ablation_command_runner_integration_step134.py"
...
$OutputRoot = "artifacts/rewards/reward_ablation_command_runner_integration_step134"
$Manifest = Join-Path $ProjectRoot "$OutputRoot\reward_ablation_command_runner_integration_step134_manifest.json"
...
if ($LASTEXITCODE -ne 0) {
    throw "[FAIL] Step 134 command-runner integration failed"
}

& $py $PyValidate --manifest $Manifest
if ($LASTEXITCODE -ne 0) {
    throw "[FAIL] Step 134 manifest validation failed"
}

& $py $PyTest
if ($LASTEXITCODE -ne 0) {
    throw "[FAIL] Step 134 self-test failed"
}

Write-Host "[DONE] Step 134 reward ablation command-runner integration complete."
