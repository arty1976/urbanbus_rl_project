...
$RunbookPath = Join-Path $AdaptersDir "causal_simulator_v2_static_signal_runbook.md"
$TestPath = Join-Path $AdaptersDir "test_causal_simulator_v2_static_signal_runbook.py"
$ProjectLogPath = ".\project_log.md"

$StartMarker = "<!-- STEP_104_CAUSAL_SIMULATOR_V2_STATIC_SIGNAL_LOG_START -->"
$EndMarker = "<!-- STEP_104_CAUSAL_SIMULATOR_V2_STATIC_SIGNAL_LOG_END -->"
...
if (-not (Test-Path $ProjectLogPath)) {
    "# urbanbus_rl_project project log`n`n$LogSection`n" | Set-Content -Path $ProjectLogPath -Encoding UTF8
} else {
    $Existing = Get-Content -Path $ProjectLogPath -Raw -Encoding UTF8
    $Pattern = [regex]::Escape($StartMarker) + ".*?" + [regex]::Escape($EndMarker)
    if ([regex]::IsMatch($Existing, $Pattern, [System.Text.RegularExpressions.RegexOptions]::Singleline)) {
        $Updated = [regex]::Replace($Existing, $Pattern, $LogSection, [System.Text.RegularExpressions.RegexOptions]::Singleline)
    } else {
        $Updated = $Existing.TrimEnd() + "`n`n" + $LogSection + "`n"
    }
    $Updated | Set-Content -Path $ProjectLogPath -Encoding UTF8
}
...
Write-Host "[OK] Step 104 files updated:"
Write-Host " - $ProjectLogPath"
...
if ($RunSelfTest) {
    ...
    & $py -m py_compile $TestPath
    if ($LASTEXITCODE -ne 0) {
        throw "[FAIL] Step 104 py_compile failed"
    }

    & $py $TestPath
    if ($LASTEXITCODE -ne 0) {
        throw "[FAIL] Step 104 self-test failed"
    }

    Write-Host "[DONE] Step 104 project log and runbook update complete."
}
