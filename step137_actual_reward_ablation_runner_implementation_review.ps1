...
$PyMain = Join-Path $RewardsDir "review_actual_reward_ablation_runner_implementation_step137.py"
$PyValidate = Join-Path $RewardsDir "validate_actual_reward_ablation_runner_implementation_review_step137.py"
$PyTest = Join-Path $RewardsDir "test_actual_reward_ablation_runner_implementation_review_step137.py"
$MdDoc = Join-Path $RewardsDir "actual_reward_ablation_runner_implementation_review_step137.md"
...
Step 137 reviews the guarded runner implementation before any future release manifest.
...
- dry-run mode must PASS.
- actual mode must return nonzero.
- actual mode manifest must be BLOCKED.
- actual mode must include actual_execution_blocked_by_step133_guard.
'@
...
$OutputRoot = "artifacts/rewards/actual_reward_ablation_runner_implementation_review_step137"
$Manifest = Join-Path $ProjectRoot "$OutputRoot\actual_reward_ablation_runner_implementation_review_step137_manifest.json"
...
if ($LASTEXITCODE -ne 0) {
    throw "[FAIL] Step 137 runner implementation review failed"
}

& $py $PyValidate --manifest $Manifest
if ($LASTEXITCODE -ne 0) {
    throw "[FAIL] Step 137 manifest validation failed"
}

& $py $PyTest
if ($LASTEXITCODE -ne 0) {
    throw "[FAIL] Step 137 self-test failed"
}

Write-Host "[DONE] Step 137 actual reward ablation runner implementation review complete."
