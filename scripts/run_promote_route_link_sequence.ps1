...
function Run-Psql {
    ...
    # psql 호출 구간에서만 ErrorActionPreference를 Continue로 전환.
    # 이유: psql이 NOTICE(알림) 등 일반 정보성 메시지를 stderr로 출력할 때,
    #       PowerShell의 2>&1 + Stop 조합이 이를 종료 오류로 오처리하는 것을 방지.
    # 실패 판정은 LASTEXITCODE(psql ON_ERROR_STOP=1)로만 판단한다.
    ...
    try {
        ...
        # $raw에는 stdout과 stderr(NOTICE 포함)가 혼합됨.
        # ErrorRecord 객체(stderr)와 문자열(stdout)을 모두 문자열로 변환하여 로그로 남긴다.
        $output = ($raw | ForEach-Object {
            if ($_ -is [System.Management.Automation.ErrorRecord]) {
                Write-Verbose "[psql stderr] $($_.Exception.Message)"
                $_.Exception.Message
            } else {
                $_
            }
