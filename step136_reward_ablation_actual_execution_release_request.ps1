...
$PyMain = Join-Path $RewardsDir "reward_ablation_actual_execution_release_request_step136.py"
$PyValidate = Join-Path $RewardsDir "validate_reward_ablation_actual_execution_release_request_step136.py"
$PyTest = Join-Path $RewardsDir "test_reward_ablation_actual_execution_release_request_step136.py"
$MdDoc = Join-Path $RewardsDir "reward_ablation_actual_execution_release_request_step136.md"
...
This source document records the purpose of Step 136.

Step 136 creates a release-request package after Step 135 readiness lock. It does not unlock actual execution.
...
The next step should create a separate release manifest validator or perform an actual runner implementation review.
'@
...
$OutputRoot = "artifacts/rewards/reward_ablation_actual_execution_release_request_step136"
$Manifest = Join-Path $ProjectRoot "$OutputRoot\reward_ablation_actual_execution_release_request_step136_manifest.json"

& $py $PyMain `
  --project-root "." `
  --output-root $OutputRoot `
  --release-reason "Prepare explicit release request package after Step 135 readiness lock."

if ($LASTEXITCODE -ne 0) {
    throw "[FAIL] Step 136 release request package failed"
}

& $py $PyValidate --manifest $Manifest
if ($LASTEXITCODE -ne 0) {
    throw "[FAIL] Step 136 manifest validation failed"
}

& $py $PyTest
if ($LASTEXITCODE -ne 0) {
    throw "[FAIL] Step 136 self-test failed"
}

Write-Host "[DONE] Step 136 reward ablation actual execution release request package complete."
