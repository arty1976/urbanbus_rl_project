...
def evaluate_replicate(*, replicate_id: str, actor: torch.nn.Module, critic: torch.nn.Module,
    ...
    return {
        ...
        "actor": {
            ...
            "ineligible_policy_contribution_abs_sum": float(actor_contribution[ineligible].detach().abs().sum().cpu()),
            "ineligible_entropy_contribution_abs_sum": float(entropy_contribution[ineligible].detach().abs().sum().cpu()),
